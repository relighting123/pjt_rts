...
export default function HourlyChart({ hourly }: Props) {
  ...
  return (
    <svg width={W} height={H} role="img">
      ...
      {[0, 0.5, 1].map((f) => (
        <g key={f}>
          <line x1={PAD.l} y1={yProd(maxProduce * f)} x2={W - PAD.r} y2={yProd(maxProduce * f)}
                stroke="#e1e6ef" />
          <text x={PAD.l - 6} y={yProd(maxProduce * f) + 4} fill="#6b7689" fontSize={10}
                textAnchor="end">
            {num(Math.round(maxProduce * f))}
          </text>
          <text x={W - PAD.r + 6} y={yUtil(f) + 4} fill="#6b7689" fontSize={10}>
            {Math.round(f * 100)}%
          </text>
        </g>
      ))}
      ...
      {/* 가동률 line */}
      <path d={utilPath} fill="none" stroke="#e0a400" strokeWidth={2} />
      {hourly.map((h, i) => (
        <circle key={h.hour} cx={xc(i)} cy={yUtil(h.util_rate)} r={3} fill="#e0a400">
          <title>{`H${h.hour} 가동률 ${pct(h.util_rate)}`}</title>
        </circle>
      ))}
      {/* x labels */}
      {hourly.map((h, i) => (
        <text key={h.hour} x={xc(i)} y={H - 8} fill="#6b7689" fontSize={10} textAnchor="middle">
          H{h.hour}
        </text>
      ))}
    </svg>
