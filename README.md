...
RULE_TIMEKEY | EQP_ID | EQP_MODEL_CD |  SEQ_NO | START_TIME | END_TIME | PLAN_PROD_KEY/ PRODUCE_QTY | CRT_TM | CRT_USER_ID | CHG_TM | CHG_USER_ID
형태로 데이터를 생성한다.
2026051723020000 형태가 Rule TImekey이고 현재 수행시간을 의미한다. 테이블 명은 RTS_RSLT_MAS / RTS_RSLT_HIS 이고 삭제후 insert하게 된다.
start time은 rule timekey과 동일 시점이 된다.

또한 BATCH_ID가 달라지는 시점에는 RTS_CONV_INF/RTS_CONV_HIS 테이블에도 저장한다.
...
아직 **미구현(후속 작업)**:
- 추론 결과를 DB에 기록하는 경로: `db.write_results`는 구현돼 있으나 호출부(추론 trace →
  출력 행 빌더)가 없고, `CHG_TM/CHG_USER_ID` 컬럼과 `RTS_CONV_INF/RTS_CONV_HIS`(BATCH_ID
  변경 시점) 기록은 미구현.
- 상단 README 예시의 일부 플래그(`--from-timekey/--to-timekey`, `--mode`,
  plan-only/wip-static/dynamic 분리)는 설계 목표이며, 현재 CLI는 `run.py`의
